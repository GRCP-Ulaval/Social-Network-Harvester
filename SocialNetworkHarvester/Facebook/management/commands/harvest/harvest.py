...
from SocialNetworkHarvester.loggers.facebookLogger import facebookLogger, logError
...
def launchFbCommentHarvestThreads(*args, **kwargs):
    ...
    for profile in profiles:
        for fbPage in profile.facebookPagesToHarvest.all():
            fbPostsToCommentHarvest = fbPostsToCommentHarvest | fbPage.fbProfile.postedStatuses.filter(
                error_on_harvest=False)
